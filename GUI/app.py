...
class App(ct.CTk):
    ...
    def __init__(self, algorithms: IAlgorithm):
        ...
        for algorithm in algorithms:
            ...
            self.algorithm_frame.grid_columnconfigure(0, weight=1)
            ...
            self.algorithm_name_label.grid(row=1, column=0, padx=20, pady=10)

            self.selected_option = tkinter.StringVar()

            self.encrypt_radio_button = ct.CTkRadioButton(master=self.algorithm_frame, text="Szyfrowanie", variable=self.selected_option, value="encrypt")
            self.encrypt_radio_button.grid(row=2, column=0, padx=20, pady=10, sticky="we")
            self.encrypt_radio_button.select()

            self.decrypt_radio_button = ct.CTkRadioButton(master=self.algorithm_frame, text="Deszyfrowanie", variable=self.selected_option, value="decrypt")
            self.decrypt_radio_button.grid(row=3, column=0, padx=20, pady=10, sticky="we")
            ...
            self.input_textbox.grid(row=5, column=0, padx=20, pady=10, sticky="we")
            ...
            self.output_textbox.grid(row=7, column=0, padx=20, pady=10, sticky="we")
            self.output_textbox.bind("<Key>", lambda e: "break")

            self.calculate_button = ct.CTkButton(master=self.algorithm_frame, text="Zaszyfruj", command=lambda i=self.input_textbox, o=self.output_textbox, a=algorithm, s=self.selected_option: self.calculate(i, o, a, s))
            self.calculate_button.grid(row=8, column=0, padx=20, pady=(15, 10), sticky="w")

            self.encrypt_radio_button.configure(command=lambda s=self.selected_option, b=self.calculate_button: self.handle_user_selection(s, b))
            self.decrypt_radio_button.configure(command=lambda s=self.selected_option, b=self.calculate_button: self.handle_user_selection(s, b))
    ...
    @staticmethod
    def calculate(input_textbox: ct.CTkTextbox, output_textbox: ct.CTkTextbox, algorithm: IAlgorithm, selected_option: tkinter.StringVar):
        output_textbox.delete("0.0", "end")

        if selected_option.get() == "encrypt":
            output_textbox.insert("end", algorithm.encrypt(input_textbox.get("0.0", "end")))
        elif selected_option.get() == "decrypt":
            output_textbox.insert("end", algorithm.decrypt(input_textbox.get("0.0", "end")))
        pass
